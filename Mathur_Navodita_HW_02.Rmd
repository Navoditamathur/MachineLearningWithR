...
Which model is the best?  
6th model (mod_lowlow_6) is ranked as the best.
...
What do you think? 
The resampling scheme did not identify the same model as the best model for the two noise cases. But the models identified to be poor for the high noise case i.e. model 9, 8, 7 remain poor for the low noise case.
...
How does it compare?  
The trend in the figure is much more clear than one presented in 1a).
...
### degree 7
set.seed(2001)
mod_highhigh_7 <- train(y~x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7), method = "lm", metric = my_metric, trControl = my_ctrl, data = df_high_high)
...
### degree 8
set.seed(2001)
mod_highhigh_8 <- train(y~x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), method = "lm", metric = my_metric, trControl = my_ctrl, data = df_high_high)
...
### degree 9
set.seed(2001)
mod_highhigh_9 <- train(y~x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8) + I(x^9), method = "lm", metric = my_metric, trControl = my_ctrl, data = df_high_high)
...
Model 5 (mod_highhigh_5) is ranked as the best.
...
What do you think?  
The resampling scheme did not identify the same model as the best model for the two sample size cases. Few models like model-9 identified to be poor for the low sample size case remain poor for the high sample size while few do not like 7, 1
...
What do you think?  
By observing the ranking of models for datasets of different sample sizes and noise levels, as sample size increases, model complexity too increases, but datasets with less noise seem to favor more complex models and datasets with more noise favor less complex models for the same sample size.
