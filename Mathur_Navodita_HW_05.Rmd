...
\implies\mu_{0} = 125
$$
On substituting $\mu_{0}$ in $\mu_{0} + 2\tau_{0} = 135$ gives
$$
 125 + 2\tau_{0} = 135\\
 \implies 2\tau_{0} = 10\\
  \implies \tau_{0} = 5
...
pnorm(mu,125,5)
```
There is 5.4% chance that the mean performance, $\mu$ is less than 117, based on my calculations of $\mu_{0}$ and $\tau_{0}$ in 1a)
...
$$
$$
\implies \frac{N}{\sigma^2}= \frac{1}{\tau_{N}^2} -\frac{1}{\tau_{0}^2} 
$$
$$
\implies N = \sigma ^{2} \left( \frac{1}{\tau_{N}^2} -\frac{1}{\tau_{0}^2} \right)
$$
Given, 
    $\sigma = 5$
From 1(a), We have $\tau_{0} = 5$

For $\tau_{N} = 2$
On substituting the value we get,
$$
N = {5^2}\left(\frac{1}{{2}^2} - \frac{1}{5^2}\right)\\
\implies N = {5^2}\frac{\left( 5^2-2^2 \right) }{2^2\times5^2}\\
\implies N = \frac{25-4}{2^2}\\
\implies N = \frac{21}{4}\\
\implies N=5.25
$$
Next, for $\tau_{N} = 0.5$
...
\implies N = {5^2}\left(\frac{100}{{5}^2} - \frac{1}{5^2}\right)\\
\implies N = {5^2}\left(\frac{99}{{5}^2}\right)\\
\implies N = 99
$$
### 1e)
...
$$
For infinitely diffuse or infinitely uncertain prior, $\tau_{0}$ is infinite
so, As $\tau_{0}$->$\infty$, 
       $\frac{1}{\tau_{0}}$->0
Therefore,
      $\frac{1}{\tau_{0}^2}$->0
...
$$
Given,
    $\tau_{N} = 3$ assuming $\sigma = 15$
On substituting the values we get,
$$
N = \frac{15^2}{3^2}\\
\implies N = \frac{225}{9}\\
\implies N=25
$$
## Problem 02
...
Estimate by taking log of likelihood 
$$
...
You may add as many equation blocks as you see fit to answer this question.  
Let posterior mode on $\mu$ be $\mu_{MAP}$
Equating the derivative to zero we get,
$$
\frac{N}{\sigma^{2}} \left( \bar{x}  - \mu_{MAP} \right) - \frac{1}{\tau_{0}^{2}}\left (\mu_{MAP}-\mu_{0}\right) = 0
$$
$$
\implies \tau_{0}^2 N\left( \bar{x}  - \mu_{MAP} \right) - \sigma^{2}\left (\mu_{MAP}-\mu_{0}\right) = 0
$$
$$
\implies  \tau_{0}^2 N \bar{x}  - \tau_{0}^2 N \mu_{MAP} - \sigma^{2}\mu_{MAP}+ \sigma^{2}\mu_{0}= 0
$$
$$
\implies  \tau_{0}^2 N \bar{x}  + \sigma^{2}\mu_{0}= \tau_{0}^2 N \mu_{MAP} + \sigma^{2}\mu_{MAP}
$$
$$
\implies \mu_{MAP}\left( \tau_{0}^2 N  + \sigma^{2}\right) = \tau_{0}^2 N \bar{x}  + \sigma^{2}\mu_{0}
$$
$$
\implies \mu_{MAP} = \frac{\tau_{0}^2 N \bar{x}  + \sigma^{2}\mu_{0}}{\left( \tau_{0}^2 N  + \sigma^{2}\right)}
$$
Dividing numerator and denominator by $\tau_{0}^2\sigma^2$, we get
$$
 \mu_{MAP} = \frac{\frac{\tau_{0}^2 N\bar{x} + \sigma^{2}\mu_{0}}{\tau_{0}^2\sigma^2} }{\frac{\left( \tau_{0}^2 N  + \sigma^{2}\right)}{{\tau_{0}^2\sigma^2}}}\ 
$$
$$
\implies \mu_{MAP} = \frac{\frac{N\bar{x}}{\sigma^2}+\frac{\mu_{0}}{\tau_{0}^2}}{\frac{N}{\sigma^{2}}+\frac{1}{\tau_{0}^2}}
$$
$$
\implies \mu_{MAP} = \frac{\frac{1}{\tau_{0}^2}\mu_{0} +\frac{N}{\sigma^2}\bar{x}}{\frac{1}{\tau_{0}^2}+\frac{N}{\sigma^{2}}}
$$
which is a weighted average of the prior and likelihood.
...
The obtained expression is similar to the posterior mean. We’re looking at a normal distribution model right now. As a result, the maximum likelihood probability would be in the middle, and the mean and mode would be aligned. As a result, the posterior mode expression is identical to the posterior mean expression.
...
$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto \log \left( \prod_{n=1}^{N} \left(\mathrm{normal}\left( x_n \mid \mu, \sigma \right) \right) \times  \mathrm{Exp}\left( \sigma \mid \lambda \right) \right)
$$
$$
\implies \log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto \log \left( \prod_{n=1}^{N} \left(\mathrm{normal}\left( x_n \mid \mu, \sigma \right) \right) \right) +\log \mathrm{Exp}\left( \sigma \mid \lambda \right) 
$$
Let us simplify the 2 expressions separately
Solving $\log \left( \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \right)$
Taking log inside, product changes to sum,
$$
\log \left( \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \right) = \sum_{n=1}^{N} \left( \log \left(   \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \right)
$$
$\mathrm{normal} \left( x_n \mid \mu, \sigma \right)$ is given by ${1/\sqrt{2\pi \sigma^{2}}}e^{{-1/2\sigma^{2}}{\left(x-\mu\right)}^2}$
...
\log \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) =  - \left(\frac{1}{2}\left( \log \left( 2\pi \right) + 2\log \sigma \right)\right) - {\frac{1}{2\sigma^{2}}{\left(x_n-\mu\right)}^2}
$$
On substituting the value of $\log \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right)$ in $\log \left( \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \right) )$ we get,
$$
\log \left( \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \right)  =  \sum_{n=1}^{N} \left(  - \left(\frac{1}{2}\left( \log \left( 2\pi \right) + 2\log \sigma \right)\right) - \frac{\left(x_n-\mu\right)^2}{2\sigma^{2}} \right)
$$
$$
\implies \log \left( \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \right)  = \sum_{n=1}^{N} \left(  - \left(\frac{1}{2} \log \left( 2\pi \right) \right) \right) + \sum_{n=1}^{N} \left( - \left(\frac{1}{2} \right) 2\log \sigma \right) - \sum_{n=1}^{N} \frac{\left(x_n-\mu\right)^2}{2\sigma^{2}}
$$
$$
\implies \log \left( \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \right)  =   - \left(\frac{1}{2} \log \left( 2\pi \right) \sum_{n=1}^{N}1 \right)  - \left( \log \sigma  \sum_{n=1}^{N}  1 \right)- \frac{1}{2\sigma^{2}}\sum_{n=1}^{N}  {\left(x_n-\mu\right)}^2
$$
Dropping the terms that does not contain $\mu$, we get
$$
\log \left( \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \right)  \propto   - \left( N \log \sigma   \right) - \frac{1}{2\sigma^{2}}\sum_{n=1}^{N} \left( x_n-\mu \right) ^2
$$
Next, 
$$
\log \mathrm{Exp}\left( \sigma \mid \lambda \right)  = \log \lambda \exp \left(-\lambda \sigma \right)
$$
$$
\implies \log \mathrm{Exp}\left( \sigma \mid \lambda \right)  = \log \lambda  + \log \exp \left(-\lambda \sigma \right)
$$
$$
\implies \log \mathrm{Exp}\left( \sigma \mid \lambda \right)  = \log \lambda -\lambda \sigma 
$$
Dropping the constant terms we get,
$$
\log \mathrm{Exp}\left( \sigma \mid \lambda \right)  \propto  -\lambda \sigma 
$$
On substituting the values of $\log \left( p\left(\mu \mid \mathbf{x}, \sigma\right) \right)$ and  $\log \mathrm{Exp}\left( \sigma \mid \lambda \right)$ in $\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right)$ we get,
$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto  - N \log \sigma - \frac{1}{2\sigma^{2}} \sum_{n=1}^{N} \left( x_n-\mu \right) ^2 -\lambda \sigma 
$$
### 3b)
...
$$
$$
\implies  \sum_{n=1}^{N}\left( \left(x_n - \mu\right)^2 \right) = Nv
$$

On substituting tha value of $\sum_{n=1}^{N}\left( \left(x_n - \mu\right)^2 \right)$in 3(a) result we get, 
$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto  -  N \log \sigma  - \frac{Nv}{2\sigma^{2}} -\lambda \sigma 
$$
I think, v is called sufficient statistic as summarizes all of the information in a sample about $\sigma$.     Yes, my expression for the un-normalized log-posterior depends on the individual observations after making use of $v$
...
{
  # your code here
  log_post = -((N*log(sigma)) + (lambda*sigma) + ((N*v)/(2*(sigma^2))))
}
```
...
Log-posterior has only one mode but the shape of the log-posterior does not look like a parabola much.
...
$$
$$
\implies g^{-1} \left(\varphi \right) = \sigma \\
or, \sigma = \exp \left( \varphi \right)
$$
So, Inverse link function is given by $\mathrm{exp}$

On substituting the same in log derivative we have,

$$
\log \left( \left| \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right| \right) = \log \left( \left| \frac{d}{d\varphi} \left( \exp \left( \varphi \right) \right) \right| \right)
$$
The derivative of $\exp$ is $\exp$ itself

$$
\log \left( \left| \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right| \right) = \log \exp \left( \varphi  \right) 
$$
$$
\implies \log \left( \left| \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right| \right) =  \varphi
$$
### 3f)
...
```
What do you think?  
Yes, the log-posterior appears to be more parabolic now as compared to earlier
...
{
  ### 
  f = -exp((x - as.double(constants['a']))^as.double(constants['m']))
  return(f)
}
...
By default optim performs minimization, but it will maximize if control$fnscale is negative. fnscale `control` argument in `optim()` tells `optim()` to maximize instead of minimize.
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x = x, 
                       y = my_func(x,my_constants))) +
  geom_line() +
  geom_vline(mapping = aes(xintercept = as.double(opt_result['par'])), color="red")
```
...
varphi_opt_b
```
No, the posterior mode is not different from the initial guess of 1.35 different from the initial guess of -0.5.
...
tibble::tibble(
  varphi = seq(log(0.5),log(4.5), length.out = 101)
   ...
   ggplot(mapping = aes(x=varphi,y=log_post)) +
   geom_line() +
   geom_vline(mapping = aes(xintercept=as.double(varphi_opt_a['par'])), color="red")
```
