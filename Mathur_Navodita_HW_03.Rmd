...
What do you think?  
The sum of value in event and non-event column for each row is 1. As value in 'event' column increases, the value in 'non-event' column  decreases.
...
*HINT*: Which variable in the `test_df_1` tibble corresponds to the observed outcome?  
 bin (0.9,1] consists of only events
...
```{r}
intervals = findInterval(test_df_1_b$event, breaks, all.inside = T)
my_calcurve_1 <- test_df_1_b%>%
  mutate(mid_bin = (breaks[intervals] + breaks[intervals + 1]) / 2)
```
```{r}
my_calcurve_1 <- my_calcurve_1%>%
  group_by(pred_bin) %>%
  mutate(prop_event = sum(y == "event")/(sum(y == "event") + sum(y == "non_event")))
```
```{r}
head(my_calcurve_1)
```
...
my_calcurve_1 %>%
  ...
  coord_equal(xlim = c(0,1),ylim = c(0,1))
```
Yes, the calibration curve is consistent with your stacked filled barchart created previously
...
Add as many code chunks as you feel are necessary.  
```{r}
intervals_3 = findInterval(test_df_3_b$event, breaks, all.inside = T)
my_calcurve_3 <- test_df_3_b%>%
  mutate(mid_bin = (breaks[intervals_3] + breaks[intervals_3 + 1]) / 2)
```
```{r}
...
Add as many code chunks as you feel are necessary. 
```{r}
intervals_8 = findInterval(test_df_8_b$event, breaks, all.inside = T)
my_calcurve_8 <- test_df_8_b%>%
  mutate(mid_bin = (breaks[intervals_8] + breaks[intervals_8 + 1]) / 2)
```
```{r}
...
What do you think?  
A well calibrated model is one whose calibrated curve is closest to 45 degree line, which is model 3 in this case. Yes, it is easy to see which model performs better using this approach. Uncertainty is the most obvious aspect of performance based on calibration curves.
...
caret::calibration(y~event,data=test_df_1,cuts = 10) %>%
  xyplot()
```
Yes, the created calibration curve is consistent with the manually created curve from the previous problem.
...
caret::calibration(y~event,data=test_df_8,cuts = 10) %>%
  xyplot()
```
Yes, the created calibration curve is consistent with the manually created curve from the previous problem (except for bins with 0 observed events).
