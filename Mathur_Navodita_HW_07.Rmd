...
calc_mae_from_laplace <- function(mvn_result, Xtrain, Xtest, y_train, y_test, num_samples)
{
  ...
  error_train <- as.matrix(abs(pred_train$Umat - y_train))
  
  # calculate the error between the test set predictions
  # and the test set observations
  error_test <- as.matrix(abs(pred_test$Umat - y_test))
...
### 
all_spline_mae_results%>%
  ggplot(mapping = aes(x=J, y=mae, color=dataset))+
  stat_summary(geom = 'line', fun = 'median')+
  scale_color_brewer(palette = "Set1")
```

Which degrees-of-freedom overfit?  
Degrees-of-freedom above 8 overfit the training data
...
From visualizations, model 7 can be considered as best.
...
What do you think?  
Model 7 is considered as best in both train/test split and the Evidence-based assessments.