...
```{r, solution_05d}
set.seed(1234)
enet_caret_tune <- train(outcome~((x2+x3+x2*x3+I(x2^2)+I(x3^2))*x1), data = df_caret , method='glmnet', metric = 'Accuracy', preProcess=c("center", "scale"), trControl = my_ctrl, tuneGrid=enet_grid, xTrans = "log" )
```
...
```{r}
enet_caret_tune$bestTune
```

What do you think?  
Ridge Regression as alpha is leaning towards 0
...
What do you think?  
Yes, the complex model H has been converted to a simpler model
...
```{r, solution_06a}
set.seed(1234)
nnet_default <- caret::train(outcome ~ ., data = df_caret , method='nnet', metric = my_metric, preProcess=c("center", "scale"),trControl = my_ctrl, trace=FALSE)
```
...
pred_viz_nnet_probs <- predict(nnet_default, viz_grid, type = 'prob')
```
...
```{r, solution_06d}
set.seed(1234)
rf_default <- caret::train(outcome ~ ., data = df_caret , method='nnet', metric = my_metric, preProcess=c("center", "scale"),trControl = my_ctrl, trace=FALSE)
```
...
```{r}
set.seed(1234)
rf_default <- caret::train(outcome ~ ., data = df_caret , method='nnet', metric = my_metric, preProcess=c("center", "scale"),trControl = my_ctrl, trace=FALSE, importance=TRUE)
```
...
What do you think?  
Predictions of the models are quite close to each other.
