...
The median price increases as color of diamond changes from D to J, while the number of diamonds with increased price decreases. 
...
### your code here
diamonds %>%
  ggplot(mapping = aes(x=color,y=price, fill=carat > median(carat))) +
  geom_boxplot() +
  facet_grid(clarity ~ cut) +
...
diamonds %>%
  ...
  ggplot(mapping = aes(x=color,y=price, fill=carat > median(diamonds$carat))) +
  geom_boxplot() +
  facet_grid(clarity ~ cut) +
  theme(legend.position="top")
```
...
2f plot shows that on one hand as the color changes from 'D' to 'G', the number of diamonds with carat greater than median carat increases  as does it's range while the median price decrases, on the other hand the number of diamonds with carat less than median number is significant only for color 'D' and its range (including outliers) keeps getting shorter as we move from color 'D' to 'G' for cut 'Ideal' and clarity 'IF'. 2b plot shows that the median price increases slightly as color of diamond changes from D to J, while the number of diamonds with increased price decreases. 
...
mpg %>%
  ggplot(mapping = aes(x=displ,y=hwy)) +
  geom_point() +
  geom_smooth(method = "lm", formula = 'y ~ x', aes(colour=drv))
```
...
### your code here
library(coefplot)
coefplot::coefplot(model=mod1)
```
As the zero is not within confidence levels, 'displ' variable is significant
...
```{r, solution_04d}
### your code here
coefplot::coefplot(model=mod2)
```
Yes, the variables are all significant. Along with intercept coefficients, 3 other coefficients are displayed.
