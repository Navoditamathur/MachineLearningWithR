...
What do you think?  
The MLE calculated in 1b) for this player's hit probability, based on the data provided is equal to that of average derived in 1a)
...
$$
\log \left( p \left( x_n \mid \mu \right) \right) = \log \left( \mu^{x _n} \left( 1-\mu \right) ^{1-x_n} \right)
$$
$$
\implies \log \left( p \left( x_n \mid \mu \right) \right) = \log \mu ^{x_n}  + \log \left( 1 - \mu \right) ^{1-x_n} 
$$
$$
\implies \log \left( p \left( x_n \mid \mu \right) \right) = x_n \log \mu + \left( 1-x_n \right) \log \left( 1-\mu \right)
$$
...
log_bernoulli_pmf <- function(xobs, prob)
{
  ...
  for (i in 1:length(xobs)){
    lbpmf[i] <- (xobs[i]*log(prob) + ((1-xobs[i])*(1-prob)))
  }
  return(lbpmf)
}
```
...
```{r}
 log_bernoulli_pmf(c(x[1]),p)
 log_bernoulli_pmf(c(x[2]),p)
 log_bernoulli_pmf(c(x[3]),p)
 log_bernoulli_pmf(c(x[4]),p)
 log_bernoulli_pmf(c(x[5]),p)
...
Insert code chunks.  
```{r}
 log_bernoulli_pmf(x[c(1:5)],p)
```

The values are equal to the previous question where we called the function separately for each observation
...
$$
p \left( \mathbf{x} \mid \mu \right) = p \left(x_1, x_2, ... , x_n, , ..., x_{N-1}, x_N \mid \mu \right)
$$

$$
p \left( \mathbf{x} \mid \mu \right) = p \left( x_1 \mid \mu \right) \times p \left( x_2 \mid \mu \right) ..... \times p \left( x_n \mid \mu \right) \times ..... p \left( x_{N-1} \mid \mu \right) \times p \left( x_N \mid \mu \right)
$$
$$
\implies \log \left( p \left( \mathbf{x} \mid \mu \right) \right) = \log \left( \prod \limits_{n=1}^N \left( \mu^{x _n} \left( 1-\mu \right) ^{1 - x_n} \right) \right)
$$

$$
\implies \log \left( p \left( \mathbf{x} \mid \mu \right) \right) = \sum \limits _{n=1}^N \log \left( \mu^{x _n} \left( 1-\mu \right) ^{1 - x_n} \right)
$$

$$
\implies \log \left( p \left( \mathbf{x} \mid \mu \right) \right) = \sum \limits_{n=1}^N \left( \log \left( \mu ^{x _n}\right) + \log \left( \left( 1-\mu \right) ^{1-x_n} \right) \right)
$$

$$
\implies \log \left( p \left( \mathbf{x} \mid \mu \right) \right) = \sum\limits_{n=1}^N \left( x _n\log \mu + \left( 1 - x_n \right) \log \left( 1-\mu \right) \right)
$$

$$
\implies \log \left( p \left( \mathbf{x} \mid \mu \right) \right) = \sum\limits_{n=1}^N \left( x _n\log \mu \right) + \sum\limits_{n=1}^N \left( \left (1-x_n \right) \log \left( 1-\mu \right) \right)
$$

$$
\implies \log \left( p \left( \mathbf{x} \mid \mu \right) \right) = \log\mu \sum\limits_{n=1}^N{x _n} + \log \left( 1-\mu \right) \sum\limits_{n=1}^N{ \left( 1-x_n \right) }
$$

$$
\implies \log \left( p \left( \mathbf{x} \mid \mu \right) \right) = \left( \log \mu  \times M \right) + \left( \log \left( 1-\mu \right) \times \left( N-M \right) \right)
$$
$$
\implies \log \left( p \left( \mathbf{x} \mid \mu \right) \right) = M \log \mu  + \left( N-M \right) \log \left( 1-\mu \right)
$$
### 2b)
...
```{r}
log_p = M*log(prob) + (N-M)*log(1-prob)
log_p
```
...
for( i in 1:length(mu_grid)){
  log_lik_xa[i] <- log(mu_grid[i])*sum(x) + log(1-mu_grid[i])*(length(x)-sum(x))
}
```
...
The curve is near flat and at its highest point, critical point near the MLE compared to other candidate values.
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x=mu,y=log_lik)) +
  geom_line(size=1.1)+
  geom_vline(xintercept=x_mle,size=1,linetype='dashed',color='red')
```
...
What do you think?  
The figure in 3c) is similar to the figure in 2f)
...
What do you think?  
The figure in 3e) is similar to the figure in 3c) and 2f)
...
Insert code chunks to answer the question. 
```{r}
prior_n_trials_uniform = a_uniform
prior_n_trials_uniform
```

```{r}
prior_n_trials_inform = a_inform
prior_n_trials_inform
```
...
```{r}
###Uniform
prior_ev_uniform = a_uniform/(a_uniform+b_uniform)
prior_ev_uniform
```

```{r}
###Inform
prior_ev_inform = a_inform/(a_inform+b_inform)
prior_ev_inform
```
...
What do you think?  
Uniform set is a straight line with no bias. There are no event probability values that are "ruled out" by the uniform set. However, the inform set rules out probability values less than 0.0625 and values greater than 0.5.
...
Posterior Beta distribution is given by distribution proportional to $\log \left( p \left( m \mid N, \mu \right) \right) + \log \left( p \left( \mu \mid a, b \right) \right)$.

Let it denoted by $p\left(\mu \mid m, N \right)$
$$ 
\log \left( p \left( \mu \mid m, N \right) \right) \propto \log \left( \mu^{m} \left( 1-\mu \right) ^{N-m} \right) + \log \left( \mu^{a-1} \left( 1-\mu \right) ^{b-1} \right)
$$

$$
\implies \log \left( p \left( \mu \mid m, N \right) \right) \propto \log \left( \mu^{m} \right) + \log \left( \left( 1-\mu \right) ^{N-m}\right) + \log \left( \mu^{a-1} \right) + \log \left( \left( 1-\mu \right) ^{b-1} \right)
$$
$$
\implies \log \left( p \left( \mu \mid m, N \right) \right) \propto m \log {\mu} + \left( N-m \right) \log \left(1-\mu \right) + \left( a-1 \right) \log {\mu} +\left( b-1 \right) \log { \left( 1- \mu\right)}
$$

$$
\implies \log \left( p\left( \mu \mid m, N \right) \right) \propto \left( m+a-1 \right) \log{\mu} + \left( b+N-m-1 \right) \log { \left( 1- \mu \right) }
$$

$$
\implies \log \left( p \left(\mu \mid m, N \right) \right) \propto \log \left( \mu^{m+a-1} \right) + \log \left( \left( 1-\mu \right) ^{b+N-m-1} \right)
$$
$$
\implies \log \left( p \left( \mu \mid m, N \right) \right) \propto \log \left( \mu^{ \left( m+a \right) -1} \right) + \log \left( \left( 1-\mu \right)^{ \left( b+N-m \right) -1} \right)
$$
Replace a+m with $a_{new}$ and b+N-m with $b_{new}$

$$
\implies \log \left( p \left( \mu \mid m, N \right) \right) \propto \log \left( \mu^{a _{new}-1} \right) + \log \left( \left( 1-\mu \right) ^{b_{new}-1} \right)
$$

$$
\implies \log \left( p \left( \mu \mid m, N \right) \right) \propto \log \left( \mu^{a _{new}-1} \left(1-\mu \right)^{b_{new}-1} \right)
$$
On comparing it with logarithm of standard beta distribution $\log \left( \mu^{a-1} \left( 1-\mu \right) ^{b-1} \right)$, we get,
...
$$ 
\frac{d}{d\mu} \log \left( \mu^{a _{new}-1} \left( 1-\mu \right) ^{b_{new}-1} \right) = \frac{d}{d\mu} \log \left( \mu ^{a _{new}-1} \right) + \frac{d}{d\mu} \log \left( 1-\mu \right) ^{b _{new}-1}
$$
$$
\implies \frac{d}{d\mu} \log \left( \mu^{a _{new}-1} \left( 1-\mu \right) ^{b_{new}-1} \right) = \left( a _{new}-1 \right) \frac{d}{d\mu} \log \mu + \left( b _{new}-1 \right) \frac{d}{d\mu} \log \left( 1-\mu \right)
$$
Taking $1-\mu$ as u, we get $\frac{d}{d\mu}$ as $\frac{d}{du}\left( \log u\right) \frac{du}{d\mu}$
Substitute u as $1-\mu$ in $\frac{du}{d\mu}$
Then,
$$
\frac{d}{d\mu}= \frac{d}{du} \log u \times \frac{d \left( 1- \mu \right)}{d\mu}
$$
$$
\implies \frac{d}{d\mu} = \left( 1/u \right) \times (-1)
$$
On substituting u = $1-\mu$, we get,
$$
\frac{d}{d\mu} = \left( 1/\left(1-\mu \right) \right) \times (-1)
$$
$$
\implies \frac{d}{d\mu} = - 1/\left(1-\mu \right) 
$$
Substituting for $\frac{d}{d\mu}\left( \log{\left( 1-\mu \right)}\right)$, we get

$$
 \frac{d}{d\mu} \log \left( \mu^{a _{new}-1} \left( 1-\mu \right) ^{b_{new}-1} \right) = \frac{a _{new}-1}{\mu} + \left( b _{new}-1 \right) \frac{1}{1-\mu} \left( -1 \right)
$$
$$
\implies \frac{d}{d\mu} \log \left( \mu^{a _{new}-1} \left( 1-\mu \right) ^{b_{new}-1} \right) = \frac{a _{new}-1}{\mu} - \frac{b _{new}-1}{1-\mu}
$$
...
Denoting posterior mode as $\mu_{MAP}$ and solving for $\frac{d}{d\mu} \log \left( \mu^{a _{new}-1} \left( 1-\mu \right) ^{b_{new}-1} \right)$ to zero, we get,

$$ 
 \frac{a _{new}-1}{\mu_{MAP}} - \frac{b _{new}-1}{1-\mu_{MAP}} = 0
$$
$$
\implies \left( 1-\mu_{MAP} \right) \left(a _{new}-1 \right) - \mu_{MAP} \left( b _{new}-1 \right) = 0
$$
$$
\implies \left( a _{new}-1 \right) - \left( \mu_{MAP} \right) \left( a _{new}-1 \right) - \mu_{MAP} \left( b _{new}-1 \right) = 0
$$
$$
\implies a _{new}-1 =  \mu_{MAP}  \left( a _{new}-1 \right) + \mu_{MAP} \left(b _{new}-1 \right)
$$
$$
\implies a _{new}-1 = \mu_{MAP} \left( a _{new}-1 + b _{new}-1 \right)
$$
$$
\implies \mu_{MAP} = \frac{a _{new}-1}{a _{new}+ b _{new}-2}
$$
...
```{r}
quantiles_uniform_new = qbeta(c(0.05,0.5,0.95), a_uniform_new, b_uniform_new)
mode_uniform_new = (a_uniform_new-1)/(a_uniform_new + b_uniform_new -2)
as_tibble_row(c(mean = quantiles_uniform_new[2],mode = mode_uniform_new, '5th percentile' = quantiles_uniform_new[1], '95th percentile' = quantiles_uniform_new[3]))
```
```{r}
quantiles_inform_new = qbeta(c(0.05,0.5,0.95), a_inform_new, b_inform_new)
mode_inform_new = (a_inform_new-1)/(a_inform_new + b_inform_new -2)
as_tibble_row(c(mean = quantiles_inform_new[2], mode = mode_inform_new, '5th percentile' = quantiles_inform_new[1], '95th percentile' = quantiles_inform_new[3]))
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, a_uniform_new, b_uniform_new))%>%
  ggplot( mapping = aes(x=mu, y=beta_pdf)) +
  geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, a_inform_new, b_inform_new))%>%
  ggplot( mapping = aes(x=mu, y=beta_pdf)) +
  geom_line()
```
...
Posterior belief based on uniform set has more uncertainty associated as compared to inform set.
...
Add as many code chunks as you feel are necessary.  
```{r}
a_uniform_season <- a_uniform + season_hits
b_uniform_season <- b_uniform + season_atbats - season_hits
```

```{r}
a_inform_season <- a_inform + season_hits
b_inform_season <- b_inform + season_atbats - season_hits
```
...
```{r}
quantiles_uniform_season = qbeta(c(0.05,0.5,0.95), a_uniform_season, b_uniform_season)
mode_uniform_season = (a_uniform_season-1)/(a_uniform_season + b_uniform_season -2)
as_tibble_row(c(mean = quantiles_uniform_season[2],mode = mode_uniform_season, '5th percentile' = quantiles_uniform_season[1], '95th percentile' = quantiles_uniform_season[3]))
```

```{r}
quantiles_inform_season = qbeta(c(0.05,0.5,0.95), a_inform_season, b_inform_season)
mode_inform_season = (a_inform_season-1)/(a_inform_season + b_inform_season -2)
as_tibble_row(c(mean = quantiles_inform_season[2],mode = mode_inform_season, '5th percentile' = quantiles_inform_season[1], '95th percentile' = quantiles_inform_season[3]))
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu,a_uniform_season, b_uniform_season))%>%
  ggplot( mapping = aes(x=mu, y=beta_pdf)) +
  geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu,a_inform_season, b_inform_season))%>%
  ggplot( mapping = aes(x=mu, y=beta_pdf)) +
  geom_line()
```
...
When the sample size is small, the prior has a greater influence on the posterior distribution compared to when the sample size is large.
It is so a with a small sample size, the amount of information contained in the sample is limited and thus, the prior has a greater impact.When the sample size is large, the sample provides a more accurate estimate of the parameter, which reduces the influence of the prior on the posterior distribution. 
